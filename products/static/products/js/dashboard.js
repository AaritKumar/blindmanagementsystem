document.addEventListener('DOMContentLoaded', function() {
    class Dashboard {
        ...
        initFormValidation() {
            ...
            this.updateCharCounter(name, 200);
            this.updateCharCounter(description, 1000);

            name.addEventListener('input', () => this.updateCharCounter(name, 200));
            description.addEventListener('input', () => this.updateCharCounter(description, 1000));
        ...
        initProductCreation() {
            ...
            form.addEventListener('submit', (event) => {
                ...
                const description = document.getElementById('id_text_description');
                let isValid = true;
                if (name.value.trim() === '') { this.showError(name, 'Product name is required.'); isValid = false; }
                if (description.value.trim() === '') { this.showError(description, 'Text description is required.'); isValid = false; }
                if (!isValid) return;
        ...
        addNewProductToDOM(product) {
            ...
            newProductDiv.className = 'grid-item';
            newProductDiv.dataset.id = product.pk;
            newProductDiv.dataset.description = product.text_description;
            ...
            newProductDiv.innerHTML = `
                ...
                <div class="item-actions">
                    ...
                    <form method="post" action="${product.delete_url}" style="display:inline;">
                        <input type="hidden" name="csrfmiddlewaretoken" value="${this.csrfToken}">
            ...
            `;
            uncategorizedGrid.appendChild(newProductDiv);

            // Add event listener for the new preview button
            newProductDiv.querySelector('.preview-btn').addEventListener('click', event => {
                const description = event.currentTarget.closest('.grid-item').dataset.description;
                this.speak(description);
            });
        }
